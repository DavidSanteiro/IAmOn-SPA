...
class PostRest extends BaseRest {
	...
	public function __construct() {
		...
		$this->postMapper = new SwitchMapper();
	...
	public function createPost($data) {
		...
		if (isset($data->title) && isset($data->content)) {
			$post->setSwitchName($data->title);
			$post->setDescription($data->content);
	...
	public function readPost($postId) {
		...
		$post_array = array(
			"id" => $post->getPublicUuid(),
			"title" => $post->getSwitchName(),
			"content" => $post->getDescription(),
			"author_id" => $post->getOwner()->getusername()

		);

		//add comments
		$post_array["comments"] = array();
		foreach ($post->getLastPowerOn() as $comment) {
	...
	public function updatePost($postId, $data) {
		...
		$post = $this->postMapper->findByPublicUUID($postId);
		if ($post == NULL) {
			header($_SERVER['SERVER_PROTOCOL'].' 400 Bad request');
			echo("Post with id ".$postId." not found");
			return;
