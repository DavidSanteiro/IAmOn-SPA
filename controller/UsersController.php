...
class UsersController extends BaseController {
	...
	* Action to login
	*
	* Logins a user checking its credentials again
	* the database
	*
	...
	* <li>posts/index: If login succeeds (via redirect)</li>
	...
	*/
	public function login() {
		if (isset($_POST["user_name"])){ // reaching via HTTP Post...
			//process login form
			if ($this->userMapper->isValidUser($_POST["user_name"], $_POST["user_password"])) {
	...
	* <li>users/login: If login succeeds (via redirect)</li>
	...
	public function register() {
		...
		if (isset($_POST["user_name"])){ // reaching via HTTP Post...
			...
			try{
				...
				if (!$this->userMapper->usernameExists($_POST["user_name"])){
					...
					$errors = array();
					$errors["general"] = "Username or mail already exists";
					$this->view->setVariable("errors", $errors);
				}
			}catch(ValidationException $ex) {
				// Get the errors array inside the exception...
				$errors["general"] = $ex->getErrors();
				// And put it to the view as "errors" variable
				$this->view->setVariable("errors", $ex->getMessage());
			}
		}
